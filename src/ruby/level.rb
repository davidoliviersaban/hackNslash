...
Squib::Deck.new(cards: level['id'].size,
  ...
  for i in 1..7 do
    ...
    text layout: 'Name', font_size: font_size, x: x+50, y: y_pos, 
        str: deck['count'+id_s].zip(deck['monster'+id_s]).map { |c,m| 
          if (c.to_s == '') # C'est epines ou bouclier
            '< ' + m.to_s + ' >'
          else
            c.to_s+' '+m.to_s 
          end
  ...
  text layout: 'Gift', str: deck['gift']
  svg layout: 'GiftIcon', file: GameIcons.get('present').file
